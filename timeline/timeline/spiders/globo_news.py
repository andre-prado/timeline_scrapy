...
class GloboNewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        data = datetime.now()
        yield {
            "title": response.xpath("normalize-space(//div[@class='highlight headline']/a/@title)").get(),
            "title_url": response.xpath("normalize-space(//div[@class='highlight headline']/a/@href)").get(),
